...
int kernel_bootcp_unlock_ddr(KBC_LOAD_TABLE_S  *table)
{
    ...
    result = trusty_kernelbootcp_call(KERNEL_BOOTCP_UNLOCK_DDR, (void *)table, insize, (uint8_t *)&out_msg, &outsize);
...
{
#ifdef NOT_VERIFY_MODEM
    KBC_IMAGE_S  *tmp_table = &(table->pm_sys);
