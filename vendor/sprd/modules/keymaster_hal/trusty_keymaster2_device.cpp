...
const uint32_t SEND_BUF_SIZE = 8192 - sizeof(struct keymaster_message) - 16;
const uint32_t RECV_BUF_SIZE = 8192;
/* The sizeof buf is 4k, but the size included header of message,
 * So the max size of data is 3960.
*/
/*lint -e681*/
const uint32_t ROUND_BUF_SIZE = (4*990);
namespace keymaster {
...
keymaster_error_t TrustyKeymasterDevice::export_key(const keymaster2_device_t* dev,
    ...
    if (client_id) {
        if (client_id->data && client_id->data_length)
           client_id_ptr = (keymaster_blob_t*)client_id;
    }
    if (app_data) {
        if (app_data->data && app_data->data_length)
           app_data_ptr = (keymaster_blob_t*)app_data;
    }

    AddClientAndAppData(client_id_ptr, app_data_ptr, &request);
...
keymaster_error_t TrustyKeymasterDevice::update(const keymaster2_device_t* dev,
    ...
    if (input) {
        ...
        ALOGE("we have %d rounds, left=%d, total=%d", round, left, input->data_length);
        *input_consumed = 0;
        if(round > 0 && output) {
    ...
    if(0 == round) {
        ...
        if (err != KM_ERROR_OK) {
            ALOGE("round=0 we send done, receive error=%d", err);
            ALOGE("Got error %d from send", err);
            return err;
        }

        if (response.error != KM_ERROR_OK) {
            ALOGE("round=0 we check resp done, receive error=%d", err);
            return response.error;
        }
    }
    ...
    for(uint32_t i = 0; i < round; i++) {
        ...
        keymaster_error_t err = Send(request, &response);
        if (err != KM_ERROR_OK) {
            ALOGE("we send done, receive error=%d", err);
            ALOGE("Got error %d from send", err);
            if(output) {
                free(tmp);
                output->data = NULL;
        ...
        if (response.error != KM_ERROR_OK) {
            ALOGE("we check resp done, receive error=%d", err);
            if(output) {
                free(tmp);
                output->data = NULL;
                output->data_length = 0;
