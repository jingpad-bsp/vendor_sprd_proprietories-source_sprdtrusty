...
const char* const ESD_CHECK = "sys/class/display/panel0/esd_check_enable";
...
int secure_display_switch(int on)
{
    ...
    if (isScreenOff()) {
        ALOGE("display not ready. light screen on first\n");
        return -2;
    }

    fd0 = open(REFRESH, O_WRONLY);
    fd1 = open(ESD_CHECK, O_WRONLY);
    fd2 = open(BG_COLOR, O_WRONLY);
    fd3 = open(DPU_IRQ_REG, O_WRONLY);
    fd4 = open(DPU_IRQ_UNREG, O_WRONLY);

    if (fd0 < 0 || fd1 < 0 || fd2 < 0 || fd3 < 0 || fd4 < 0) {
        ALOGE("secureDisplay Open display file node fail. %d, %d, %d, %d, %d\n", fd0, fd1, fd2, fd3,
              fd4);
        return -1;
    }
