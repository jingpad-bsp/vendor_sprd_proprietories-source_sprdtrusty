                               ...
                               int *addrp)
{
    ...
    for (i = 0; i < res_count; i++) {
        if (rpmb_get_u16(res[i].req_resp) != response_type) {
            LOG_ERROR("%s: Bad response type, 0x%x, expected 0x%x\n",
        ...
        if (nonce && CRYPTO_memcmp(res[i].nonce.byte, nonce->byte, sizeof(nonce->byte))) {
            LOG_ERROR("%s: Bad nonce\n", cmd_str);
            return -1;
        }

        if (addrp && *addrp != rpmb_get_u16(res[i].address)) {
            LOG_ERROR("%s: Bad addr, got %u, expected %u\n",
...
static int check_res(struct rpmbproxy_msg *msg, int res)
{
    ...
    if ((size_t)res < sizeof(*msg)) {
        LOG_ERROR("invalid msg length (%zd < %zd)\n", res, sizeof(*msg));
        return -EIO;
    }

    //LOG_INFO("cmd 0x%x: server returned %u\n", msg->cmd, msg->result);
    ...
    switch(msg->result) {
        ...
        case RPMBPROXY_ERR_NOT_VALID:
            LOG_ERROR("cmd 0x%x: parameter is invalid\n", msg->cmd);
            return -EINVAL;

        case RPMBPROXY_ERR_UNIMPLEMENTED:
            LOG_ERROR("cmd 0x%x: is unhandles command\n", msg->cmd);
            return -EINVAL;

        case RPMBPROXY_ERR_GENERIC:
            LOG_ERROR("cmd 0x%x: internal server error\n", msg->cmd);
            return -EIO;

        default:
            LOG_ERROR("cmd 0x%x: unhandled server response %u\n",
                   msg->cmd, msg->result);
    }
                         ...
                         const struct iovec *rx_iovs, uint rx_iovcnt)
{
    ...
    if (rc < 0) {
        rc = -errno;
        LOG_ERROR("failed to recv response: %s\n", strerror(errno));
        return rc;
    }
...
static int rpmbproxy_read_counter(uint32_t *write_counter)
{
    ...
    if (ret < 0) {
        LOG_ERROR("rpmb_read_counter failed\n");
        return ret;
    }

    ret = rpmbproxy_mac(&res, 1, &mac);
    if (ret < 0) {
        LOG_ERROR("rpmb_read_counter check mac failed\n");
        return ret;
    }
...
int rpmb_read(unsigned char *buf, int blk_ind, int blk_cnt)
{
    ...
    if (!init_done) {
        LOG_ERROR("not init,please call init_rpmb()\n");
        return -EINVAL;
    }

    if (blk_cnt > MAX_RPMB_BLOCK_CNT) {
        LOG_ERROR("the block count is to lager: %d max %d\n", blk_cnt, MAX_RPMB_BLOCK_CNT);
        return -EINVAL;
    }

    if (blk_ind > RPMB_BLOCK_IND_END || blk_ind < RPMB_BLOCK_IND_START) {
        LOG_ERROR("the block index %d is out of bounds: %d - %d\n",
    ...
    ret = rpmb_ops_send(mmc_handle, NULL, (size_t)0, (void *)&cmd, (size_t)sizeof(cmd), (void *)res, (size_t)sizeof(res));
    if (ret < 0) {
        LOG_ERROR("failed to write rpmb: %s\n", strerror(errno));
        return ret;
    }
    ret = rpmbproxy_mac(res, blk_cnt, &mac);
    if (ret < 0) {
        LOG_ERROR("failed to get mac: %d\n", ret);
        return ret;
    }


    LOG_INFO("rpmb: read data, addr %d, count %d, response:\n", blk_ind, blk_cnt);
...
int rpmb_write(const unsigned char *buf, int blk_ind, int blk_cnt)
{
    ...
    if (ret < 0) {
        LOG_ERROR("failed to read write counter: %d\n", ret);
        return ret;
    }
    ...
    LOG_INFO("rpmb: write data, addr %d, count %d, write_counter %d, response\n",
...
int init_rpmb()
{
    ...
    if (0 > ret){
        LOG_ERROR("init_rpmb rpmb_ops_open()=%d\n", ret);
        return ret;
    }
    mmc_handle = ret;
    ret = tipc_connect(TRUSTY_DEVICE_NAME, RPMBPROXY_PORT);
    if (0 > ret) {
        LOG_ERROR("tipc_connect(%s, %s) failed! %s\n", TRUSTY_DEVICE_NAME, RPMBPROXY_PORT, strerror(errno));
        return ret;
    }
