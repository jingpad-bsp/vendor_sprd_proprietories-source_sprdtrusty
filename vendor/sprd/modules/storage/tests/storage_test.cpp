...
//    gettimeofday(&start, NULL);
    ...
    if (rc != chunk_len) {
        dprintf("storage_write() faile, write lenth %d, expect %lu\n", (int)rc, (unsigned long)chunk_len);
    }
                                     ...
                                     size_t data_len, size_t chunk_len)
{
    ...
    while (data_len) {
        ...
        if (read_len != chunk_len){
            dprintf("storage_read() failed, read lenth %lu, expect %lu\n", (unsigned long)read_len, (unsigned long)chunk_len);
            return -1;
        } else {
...
int test_storage_read_to(const char *port,  const char *fname, char *data_buf, storage_off_t data_len, storage_off_t  offset, bool complete)
{
    storage_session_t session_;
    file_handle_t handle;
    int rc = -1;
    ...
    if ( read_len != data_len) {
        dprintf("storage_read() failed! return %ld,exp %ld\n", (long)read_len, (long)data_len);
		rc = -1;
        goto out;
    }
...
err1:
    ...
    gettimeofday(&end, NULL);
    dprintf("test_storage_read(%s, %s, file_size %ld, read %lu,) use %ld us\n", port, fname, (long)file_size, (long)read_len,
        1000000 * ( end.tv_sec - start.tv_sec ) + (end.tv_usec - start.tv_usec));
...
*/
    ...
    for (int i = 1; i < 10; i++) {
        ...
        test_storage_read_to(STORAGE_CLIENT_TO_PORT,file_name, data_buf, 4024 * 100, 0, false);
    }
/*
